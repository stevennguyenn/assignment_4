...
class CheckCodeGenSuite(unittest.TestCase):
    ...
    # def test_int_ast_int_mod(self):
    #     input = Program([
    #             FuncDecl(Id("main"),[],VoidType(),Block([
    #                 CallExpr(Id("putInt"),[BinaryOp("%",IntLiteral(5),IntLiteral(2))])]))])
    #     expect = "0"
    #     self.assertTrue(TestCodeGen.test(input,expect,506))
    def test_int_ast_int_and(self):
        input = Program([
                FuncDecl(Id("main"),[],VoidType(),Block([
                    CallExpr(Id("putBool"),[BinaryOp(">=",IntLiteral(5),IntLiteral(2))])]))])
        expect = "false"
        self.assertTrue(TestCodeGen.test(input,expect,507))
    # def test_int_ast_int_or(self):
    #     input = Program([
    #             FuncDecl(Id("main"),[],VoidType(),Block([
    #                 CallExpr(Id("putBool"),[BinaryOp("||",IntLiteral(5),IntLiteral(2))])]))])
    #     expect = "true"
    #     self.assertTrue(TestCodeGen.test(input,expect,508))
    # def test_int_ast_int_grater(self):
    #     input = Program([
    #             FuncDecl(Id("main"),[],VoidType(),Block([
    #                 CallExpr(Id("putBool"),[BinaryOp("||",BooleanLiteral(False),BooleanLiteral(False))])]))])
    #     expect = "False"
